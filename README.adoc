...
image:https://pkg.go.dev/badge/github.com/louiss0/g-tools.svg[GoDoc,link=https://pkg.go.dev/github.com/louiss0/g-tools]
image:https://goreportcard.com/badge/github.com/louiss0/g-tools.svg[Go Report Card,link=https://goreportcard.com/report/github.com/louiss0/g-tools]
...
import (
    "fmt"
    "github.com/louiss0/g-tools/enum" // Replace with your actual module path
)
...
```bash
go build -ldflags "-X github.com/louiss0/g-tools/mode.buildMode=production" main.go
```
...
import (
    "fmt"
    "github.com/louiss0/g-tools/mode" // Replace with your actual module path
)
...
=== `regex_extract` Package

The `regex_extract` package provides helpers for extracting data from regex
matches while enforcing consistent error handling and type conversion.

*   **`ExtractGroups`**: Extracts named capture groups into a `map[string]string`.
*   **`ExtractSlice`**: Extracts capture groups into a typed slice.
*   **`ExtractTypedNamedGroups`**: Extracts named groups into typed values,
    including nested captures, and validates against expected map values.
*   **`ExtractTypedUnnamedGroups`**: Extracts unnamed groups into typed slices,
    preserving nested captures.
*   **`ExtractToStruct`**: Assigns named groups into struct fields.

All extractors accept a regex pattern string and compile it internally, returning
an error when the pattern is invalid.
